...
<center>
    ...
    <table border="1" bordercolor="blue" width="400" height="100">
        <tr>
            <th>번호</th> <th>이름</th> <th>학교</th> <th>전화번호</th> <th>결석</th>
        </tr>
    <%
        for(int i = 0; i < stBean.size(); i++){
            %>
                <tr>
                    ...
                    <td>
                    <center>
                        <input type="button" value="결석" onclick="location.href='sta_absentCheck.jsp?id=<%=stBean.get(i).getStId()%>&date=<%=date %>&name=<%=stBean.get(i).getStName()%>'">
                        </center>
                    </td>
